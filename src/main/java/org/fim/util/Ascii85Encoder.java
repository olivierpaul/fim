...
// File : https://github.com/blackducksoftware/common-framework/blob/master/src/main/java/com/blackducksoftware/tools/commonframework/core/encoding/Ascii85Encoder.java
...
public class Ascii85Encoder {
	...
	if (bytesToBeEncoded.length % 4 != 0) {
	    throw new IllegalArgumentException(
		    "The byte array to encode must have a length that is divisible by 4");
	}
     ...
     */
    private static final void encodeChunk(byte[] inputChunk, byte[] outputChunk) {
	long word = ((((inputChunk[0] << 8) | (inputChunk[1] & 0xFF)) << 16)
		| ((inputChunk[2] & 0xFF) << 8) | (inputChunk[3] & 0xFF)) & 0xFFFFFFFFL;
     ...
     */
    public static byte[] decode(byte[] bytesToBeDecoded)
	    throws DecoderException {
	...
	if (bytesToBeDecoded.length % 5 != 0) {
	    throw new IllegalArgumentException(
		    "The byte array to decode must have a length that is divisible by 5");
	}
	...
	for (int bytesToBeDecodedIndex = 0; bytesToBeDecodedIndex < bytesToBeDecoded.length;) {
		...
		for (chunkOfFiveCharsToDecodeIndex = 0; chunkOfFiveCharsToDecodeIndex < 5; ++chunkOfFiveCharsToDecodeIndex) {
		    ...
		    if (offsetOfByteToBeDecoded < 0
			    || offsetOfByteToBeDecoded > MAX_ENCODED_CHAR_VALUE_OFFSET) {
			throw new IllegalArgumentException(
				"Invalid data in Ascii85 stream");
		    }
		    decodedByteValue = (decodedByteValue * 85L)
			    + offsetOfByteToBeDecoded;
		}
